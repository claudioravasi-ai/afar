...
function documentoFicha(f, opts){
  ...
  /* El titulo no puede anunciar lo que el documento no trae. La copia del
     paciente decia «VALORACION PREQUIRURGICA Y CONSENTIMIENTO INFORMADO»
     siempre, pero el consentimiento viaja como PDF aparte -es un instrumento
     juridico autonomo, se firma y se presenta solo- y adentro de este archivo
     no esta: docPacienteValoracion() lo pide con `sinConsentimiento:true`.
     Asi que el titulo se calcula con la MISMA condicion con la que se dibuja
     la seccion, mas abajo. Si algun dia el consentimiento vuelve a incluirse,
     el titulo lo dice solo; y si no, no lo nombra. */
  const conConsentimiento = !!co.quien && !(opts && opts.sinConsentimiento);

  return ''+
  membrete(f, paraPaciente
    ? ('VALORACIÓN PRE-ANESTÉSICA'+
       (conConsentimiento ? ' Y CONSENTIMIENTO INFORMADO' : '')+
  ...
  /* La MISMA condición que decide el título, arriba: una sola fuente para las
     dos cosas, para que el encabezado no pueda volver a anunciar una sección
     que no se dibuja. */
  (conConsentimiento ? seccion('Consentimiento informado anestésico',
    (soloActo ? '' :
